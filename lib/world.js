...
class World{
    constructor(canvas){
        ...
        this.scene3D        = createScene(0xd3e0ea);                                    // シーン(3D空間的な)
        // アニメーション関連
        ...
        this.character      = new Character(this.scene3D, this.type);                   // キャラクターを作成
        // ユーザー操作
        ...
        this.scene3D.add(ambientLight, mainLight);                                      // シーンにライトを追加
    ...
    Animate(){
        ...
        this.character.changeAngle(this.camera.getCurrentAngle());                      // キャラクターの角度をカメラに合わせる
        this.character.move();                                                          // キャラクターの位置を更新
        this.camera.move(delta, this.character.x, this.character.y);                    // カメラ位置の更新(注視点をキャラクターの位置に合わせる)
        this.renderer.render( this.scene3D, this.camera.getCurrentCamera() );           // 描画
    }
