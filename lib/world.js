...
class World{
    constructor(canvas){
        ...
        this.fPersonCamera  = createFirstPersonCamera(0, 0, 2.5);
        // キャラクター
        this.type           = Math.floor( Math.random() * (11 - 1) ) + 1 ;
        this.character      = new Character(this.scene3D, this.type);
        // ユーザー操作
        this.input          = new InputManager(this.character, this.camera3D, this.fPersonCamera, this.canvas);
        // 要素
    ...
    Animate(){
        ...
        this.input.update(delta, this.character.x, this.character.y);  

        // 2Dと3Dの切り替え
        if(this.isFirstView){
            if(!this.input.pControls.isLocked) this.input.pControls.lock();
            this.renderer.render( this.scene3D, this.fPersonCamera );           // 一人称バージョンで描画
        }else{
            if(this.input.pControls.isLocked) this.input.pControls.unlock();
        ...
        this.camera3D.lookAt(this.character.x, 5, this.character.y);
        //this.fCamera.position.set(this.character.x, this.character.z, this.character.y);
